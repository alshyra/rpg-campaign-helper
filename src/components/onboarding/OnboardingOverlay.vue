<template>
  <Teleport to="body">
    <transition name="fade">
      <div v-if="isVisible" class="onboarding fixed inset-0 z-[60] grid place-items-center px-3.5 py-5" role="dialog" aria-modal="true" aria-labelledby="onboarding-title">
        <div class="onboarding__backdrop"></div>
        <section class="onboarding__panel relative grid w-[min(100%,480px)] max-h-[calc(100dvh-2.5rem)] overflow-y-auto gap-5 rounded-[28px] border border-(--line-strong) bg-[linear-gradient(180deg,rgba(39,28,21,0.98),rgba(20,15,11,0.98))] px-5 py-6 shadow-[0_30px_80px_rgba(0,0,0,0.48)]">

          <!-- Étape d'intro -->
          <transition name="slide" mode="out-in">
            <div v-if="currentStep === 0" key="intro" class="grid gap-3">
            ...
            <!-- Étapes features -->
            <div v-else :key="steps[currentStep - 1].id" class="grid gap-3">
              <div class="onboarding__feature-icon grid h-12 w-12 place-items-center rounded-2xl border border-[rgba(221,187,123,0.15)] bg-white/3 text-amber-400">
                <svg viewBox="0 0 24 24" class="h-6 w-6" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" v-html="steps[currentStep - 1].icon" aria-hidden="true" />
              </div>
              <h2 :id="currentStep === 1 ? 'onboarding-title' : undefined" class="onboarding__title">{{ steps[currentStep - 1].title }}</h2>
              <p class="m-0 leading-relaxed text-(--text-soft)">{{ steps[currentStep - 1].desc }}</p>
            </div>
          </transition>
          ...
          <div class="onboarding__actions grid grid-cols-2 gap-2.5">
            <button class="secondary-button" type="button" @click="dismiss(false)">Passer</button>
            <button
