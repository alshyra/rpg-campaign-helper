<template>
  <nav class="bottom-nav fixed bottom-3.5 left-1/2 z-10 grid w-[min(92vw,528px)] -translate-x-1/2 grid-cols-3 gap-2 rounded-3xl border border-(--line) bg-[rgba(17,12,9,0.94)] p-2.5 shadow-[0_22px_50px_rgba(0,0,0,0.45)] backdrop-blur-[18px]" aria-label="Navigation principale">
    <RouterLink
      v-for="item in items"
      :key="item.to"
      :to="item.to"
    ...
    >
      <span class="bottom-nav__icon h-5.5 w-5.5" aria-hidden="true" v-html="item.icon"></span>
