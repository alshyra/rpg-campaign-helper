<template>
  <AppCard>
    <form class="character-editor grid gap-4" @submit.prevent="submitCharacter">
      ...
      <!-- Étape : Identité -->
      <div v-if="currentStepId === 'identity'" class="grid grid-cols-2 gap-3.5 max-[420px]:grid-cols-1">
        <FormField v-model="draft.profile.characterName" label="Nom" required />
        <FormField v-model="draft.profile.role" label="Rôle / classe" required />
        <FormField v-model="draft.profile.mood" label="Ambiance" type="textarea" :full="true" rows="3" />
      </div>

      <!-- Étape : Stats -->
      <div v-if="currentStepId === 'stats'" class="grid gap-3 border-t border-white/5 pt-1">
      ...
      <!-- Étape : Compétences -->
      <div v-if="currentStepId === 'skills'" class="grid gap-3 border-t border-white/5 pt-1">
        <div class="flex items-end justify-between gap-3 max-[420px]:grid max-[420px]:grid-cols-1">
          <p class="section-heading__eyebrow">Compétences</p>
          <IconButton type="button" @click="addSkill">Ajouter une compétence</IconButton>
        </div>
        <p v-if="draft.skills.length === 0" class="text-sm opacity-50">
      ...
      <div class="wizard-actions grid grid-cols-2 gap-2.5 max-[420px]:grid-cols-1">
        <Button v-if="currentStep > 0" variant="secondary" type="button" @click="previousStep">
          Précédent
        </Button>
        <Button
        ...
        <Button v-else variant="primary" type="submit">{{ submitLabel }}</Button>
      </div>
    </form>
  </AppCard>
</template>
