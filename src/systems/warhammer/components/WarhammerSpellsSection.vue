  ...
  <article class="rounded-2xl border border-white/5 bg-stone-900/40 p-4">
    ...
    >
      <div
        ...
        :key="spell.id"
        class="group rounded-xl border border-white/5 bg-black/30 p-3"
      >
        <div class="flex items-start justify-between gap-2">
          <div>
            <h4 class="m-0 font-bold text-amber-100">{{ spell.name }}</h4>
            <p
              v-if="spell.difficulty || spell.ingredients"
              class="m-0 mt-0.5 text-[10px] text-stone-500"
            >
              {{ spell.difficulty ? `Difficulté ${spell.difficulty}` : ""
              }}{{ spell.difficulty && spell.ingredients ? " · " : "" }}{{ spell.ingredients ? spell.ingredients : "" }}
            </p>
          </div>
          <IconButton
            square
        ...
        <p
          ...
          class="m-0 mt-2 text-xs leading-relaxed text-stone-400"
        >
          {{ spell.description }}
        </p>
      </div>
