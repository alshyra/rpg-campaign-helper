    ...
    <AppCard>
      <div class="start-onboarding__content relative grid gap-3.5">
        ...
        <p class="m-0 leading-relaxed text-(--text-soft)">
          Commence avec une création guidée de personnage, ou restaure une sauvegarde JSON existante.
          Toutes les données restent locales dans ton navigateur.
        </p>

        <div class="start-onboarding__actions grid gap-2.5">
          <Button variant="primary" type="button" @click="goToTunnel">Créer mon personnage</Button>
          <FileImportLabel label="Importer un personnage JSON" @file-selected="onFileChange" />
        </div>
      </div>
    </AppCard>
  </div>
